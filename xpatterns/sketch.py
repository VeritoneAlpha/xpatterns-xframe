...
class Sketch(object):
    ...
    For numeric columns, the following information is provided exactly:
     - length (:func:`~xpatterns.Sketch.size`)
     - number of missing Values (:func:`~xpatterns.Sketch.num_undefined`)
     - minimum  value (:func:`~xpatterns.Sketch.min`)
     - maximum value (:func:`~xpatterns.Sketch.max`)
     - mean (:func:`~xpatterns.Sketch.mean`)
     - variance (:func:`~xpatterns.Sketch.var`)
     - standard deviation (:func:`~xpatterns.Sketch.std`)

    And the following information is provided approximately:
     - number of unique values (:func:`~xpatterns.Sketch.num_unique`)
     - quantiles (:func:`~xpatterns.Sketch.quantile`)
     - frequent items (:func:`~xpatterns.Sketch.frequent_items`)
     - frequency count for any value (:func:`~xpatterns.Sketch.frequency_count`)
    ...
    For example:
         ...
         >>> sa = xpatterns.XArray([{'a':1, 'b':2}, {'a':3}])
    ...
    For example:
         ...
         >>> sa = xpatterns.XArray([[100,200,300,400,500], [100,200,300], [400,500]])
