...
class XFrame(object):
    ...
    data in XFrame is stored row-wise in a Spark RDD.
    memory size.  Each row of the RDD is a list, whose elements correspond
    to the values in each column.  The column names are stored in the XFrame
    ...
    * the XPatterns Server's file system
    * HDFS
    * Amazon S3
    * HTTP(S).
    ...
    def read_csv_with_errors(cls,
        ...
        >>> (xf, bad_lines) = xpatterns.XFrame.read_csv_with_errors(bad_url)
        >>> xf
        +---------+----------+--------+
    ...
    def read_csv(cls,
        ...
        >>> xf = xpatterns.XFrame.read_csv(url)
        >>> xf
        Columns:
          user_id int
          movie_id  int
        ...
        >>> xf = xpatterns.XFrame.read_csv(url, nrows=100)
        >>> xf
        Columns:
          user_id int
          movie_id  int
        ...
        >>> xf = xpatterns.XFrame.read_csv(url, column_type_hints=str)
        >>> xf
        Columns:
          user_id  str
          movie_id  str
        ...
        >>> xf = xpatterns.XFrame.read_csv(url, header=False)
        >>> xf
        Columns:
          X1  str
          X2  str
        ...
        >>> xf = xpatterns.XFrame.read_csv(url, na_values=['3'], column_type_hints=str)
        >>> xf
        Columns:
          user_id str
          movie_id  str
        ...
        >>> bad_url = 'https://s3.amazonaws.com/gl-testdata/bad_csv_example.csv'
        >>> xf = xpatterns.XFrame.read_csv(bad_url, error_bad_lines=True)
    ...
    def apply(self, fn, dtype=None, seed=None):
        """
        Transform each row to an :class:`~xpatterns.XArray` according to a
    ...
    def random_split(self, fraction, seed=None):
        ...
        >>> xf = xpatterns.XFrame({'id': range(1024)})
    ...
    def topk(self, column_name, k=10, reverse=False):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'id': range(1000)})
        >>> xf['value'] = -xf['id']
        >>> xf.topk('id', k=3)
        +--------+--------+
        |   id   |  value |
    ...
    def select_column(self, column_name):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'user_id': [1,2,3],
    ...
    def add_column(self, data, name=""):
        ...
        >>> sa = xpatterns.XArray(['cat', 'dog', 'fossa'])
    ...
    def add_columns(self, data, namelist=None):
        ...
        --------
        >>> xf = xpatterns.XFrame({'id': [1, 2, 3], 'val': ['A', 'B', 'C']})
        >>> xf2 = xpatterns.XFrame({'species': ['cat', 'dog', 'fossa'],
        ...                        'age': [3, 5, 9]})
        >>> xf.add_columns(xf2)
        >>> xf
        +----+-----+-----+---------+
        ...
        if isinstance(data, XFrame):
            ...
            for name in namelist:
                if name in my_columns:
                    raise ValueError("Column '" + name + "' already exists in current XFrame")
            with impl_context():
    ...
    def remove_columns(self, column_names):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'id': [1, 2, 3], 'val1': ['A', 'B', 'C'], 'val2': [10, 11, 12]})
    ...
    def swap_columns(self, column_1, column_2):
        ...
        if column_1 not in self.column_names():
            raise KeyError('Cannot find column %s' % column_1)
        if column_2 not in self.column_names():
            raise KeyError('Cannot find column %s' % column_2)
...
#                    yield dict(zip(column_names, j))
    ...
    def append(self, other):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'id': [4, 6, 8], 'val': ['D', 'F', 'H']})
        >>> xf2 = xpatterns.XFrame({'id': [1, 2, 3], 'val': ['A', 'B', 'C']})
        >>> xf = xf.append(xf2)
        >>> xf
        +----+-----+
        | id | val |
        ...
        for i in range(len(my_column_names)):
            if other_column_names[i] != my_column_names[i]:
                raise RuntimeError("Column " + my_column_names[i] + 
                                   " name is not the same in two XFrames, one is " + 
                                   str(my_column_names[i]) + 
                                   ", the other is " + 
                                   str(other_column_names[i]))
            # check column type
            if my_column_types[i] != other_column_types[i]:
                raise RuntimeError("Column " + my_column_names[i] + 
                                   " type is not the same in two XFrames, one is " + 
                                   str(my_column_types[i]) + 
                                   ", the other is " + 
                                   str(other_column_types[i]))
    ...
    def groupby(self, key_columns, operations, *args):
        ...
        VAR. See :mod:`~xpatterns.aggregate` for more detail on the aggregators.
        ...
        *args
        ...
        >>> import xpatterns.aggregate as agg
        >>> url = 'http://s3.amazonaws.com/gl-testdata/rating_data_example.csv'
        >>> xf = xpatterns.XFrame.read_csv(url)
        >>> xf
        +---------+----------+--------+
        ...
        for (cols, op) in zip(group_columns, group_ops):
            ...
            if op is not xpatterns.aggregate.COUNT()[0]:
                for col in cols:
    ...
    def join(self, right, on=None, how='inner'):
        ...
        Examples
        --------
        >>> animals = xpatterns.XFrame({'id': [1, 2, 3, 4],
        ...                           'name': ['dog', 'cat', 'sheep', 'cow']})
        >>> sounds = xpatterns.XFrame({'id': [1, 3, 4, 5],
    ...
    def split_datetime(self, expand_column, column_name_prefix=None, limit=None, tzone=False):
        ...
        other, refer to :py:func:`xpatterns.XArray.expand()`
    ...
    def filter_by(self, values, column_name, exclude=False):
        ...
        given ``values`` :class:`~xpatterns.XArray`. If ``values`` is not an
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'id': [1, 2, 3, 4],
    ...
    def pack_columns(self, columns=None, column_prefix=None, dtype=list,
        ...
        >>> xf = xpatterns.XFrame({'business': range(1, 5),
        ...
        if new_column_name != None:
            ...
            if new_column_name in rest_columns:
                raise KeyError("Current XFrame already contains a column name " + new_column_name)
        else:
            new_column_name = ""
    ...
    def unpack(self, unpack_column, column_name_prefix=None, column_types=None,
        ...
        :py:func:`~xpatterns.XArray.unpack()`.
    ...
    def stack(self, column_name, new_column_name=None, drop_na=False):
        ...
        >>> xf = xpatterns.XFrame({'topic':[1,2,3,4],
        ...                       'words': [{'a':3, 'cat':2},
        ...                                 {'a':1, 'the':2},
        ...                                 {'the':1, 'dog':3},
        ...                                 {}]
        ...
        >>> xf = xpatterns.XFrame({'topic':[1,2,3],
        ...                       'friends':[[2,3,4], [5,6],
        ...                                  [4,5,10,None]]
        ...                      })
        >>> xf
    ...
    def unstack(self, column, new_column_name=None):
        ...
        groupby : ``unstack`` is a special version of ``groupby`` that uses the
          :mod:`~xpatterns.aggregate.CONCAT` aggregator
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'count':[4, 2, 1, 1, 2, None],
        ...
        >>> xf = xpatterns.XFrame({'friend': [2, 3, 4, 5, 6, 4, 5, 2, 3],
    ...
    def unique(self):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'id':[1,2,3,3,4], 'value':[1,2,3,3,4]})
        >>> xf
        +----+-------+
        | id | value |
        +----+-------+
    ...
    def dropna(self, columns=None, how='any'):
        ...
        >>> xf = xpatterns.XFrame({'a': [1, None, None], 'b': ['a', 'b', None]})
        >>> xf.dropna()
        +---+---+
        | a | b |
        +---+---+
    ...
    def dropna_split(self, columns=None, how='any'):
        ...
        same functionality as :py:func:`~xpatterns.XFrame.dropna`, but returns a
        tuple of two XFrames.  The first item is the expected output from
        :py:func:`~xpatterns.XFrame.dropna`, and the second item contains all the
    ...
    def fillna(self, column, value):
        ...
        Examples
        --------
        >>> xf = xpatterns.XFrame({'a':[1, None, None],
        ...                       'b':['13.1', '17.2', None]})
        >>> xf = xf.fillna('a', 0)
        >>> xf
        +---+------+
