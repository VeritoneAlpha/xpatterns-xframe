...
# There have been a few minor changes and one bug fix.
# See license notice in main module.
...
class HeapDict(collections.MutableMapping):
    ...
    def popitem(self):
        """
        D.popitem() -> (k, v), remove and return the (key, value) pair
           with lowest value; but raise KeyError if D is empty.
        """
        wrapper = self.heap[0]
        if len(self.heap) == 1:
            self.heap.pop()
    ...
    def peekitem(self):
        """
        D.peekitem() -> (k, v), return the (key, value) pair
            with lowest value; but raise KeyError if D is empty.
        """
